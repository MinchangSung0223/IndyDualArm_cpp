...
namespace mr {
	...
	RelativePinvJacobian RelativePinvJacobianBody(const RelativeScrewList& relBlist, const RelativeJVec& thetaList) {
		...
		double epsilon = 0.0000001;
		//Eigen::JacobiSVD<RelativeJacobian> svd(relJb,Eigen::ComputeFullU | Eigen::ComputeFullV);
		//Eigen::JacobiSVD<RelativeJacobian> svd(relJb ,Eigen::ComputeThinU | Eigen::ComputeThinV);
		//double tolerance = epsilon * std::max(relJb.cols(), relJb.rows()) *svd.singularValues().array().abs()(0);
		//cout<<relJb.cols()<<endl;
		//cout<<relJb.rows()<<endl;
		//RelativePinvJacobian pinv_relJb;
 		//pinv_relJb=  svd.matrixV() *  (svd.singularValues().array().abs() > tolerance).select(svd.singularValues().array().inverse(), 0).matrix().asDiagonal() * svd.matrixU().adjoint();
	...
	RelativePinvJacobian RelativePinvJacobianSpace(const RelativeScrewList& relSlist, const RelativeJVec& thetaList) {
		...
		//double epsilon = 0.0000001;
		//Eigen::JacobiSVD<RelativeJacobian> svd(Js ,Eigen::ComputeThinU | Eigen::ComputeThinV);
		//double tolerance = epsilon * std::max(Js.cols(), Js.rows()) *svd.singularValues().array().abs()(0);
 		//RelativePinvJacobian pinv_relJs =  svd.matrixV() *  (svd.singularValues().array().abs() > tolerance).select(svd.singularValues().array().inverse(), 0).matrix().asDiagonal() * svd.matrixU().adjoint();
